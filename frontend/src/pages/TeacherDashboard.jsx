...
const TeacherDashboard = () => {
    ...
    useEffect(() => {
        ...
        if (hash && ['overview', 'classes', 'attendance', 'timetable', 'marks', 'profile'].includes(hash)) {
            setActiveTab(hash);
        }
    }, [location.hash]);
    ...
    const handleGenerateQR = async () => {
        try {
            await api.post('/teacher/generate-qr', {
            ...
            setShowQR(true);
            setScannedStudents([]); // Reset for new session
        } catch (err) {
    ...
    const handleRemoveAttendance = async (studentId) => {
        if (!window.confirm("Remove this student from today's attendance?")) return;
        try {
            await api.delete(`/teacher/attendance/${studentId}/${selectedSubject}/${selectedSlot}`);
    ...
    const handleStopSession = async () => {
        try {
            await api.post('/teacher/stop-session');
            setShowQR(false);
            setQrCode("");
            setScannedStudents([]);
        } catch (err) {
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex bg-slate-900/50 p-1 rounded-2xl border border-slate-800">
                    {['overview', 'classes', 'attendance', 'timetable', 'marks', 'profile'].map((tab) => (
                        <button
                            key={tab}
                        ...
                        <div className="space-y-4">
                            <div className="grid grid-cols-3 gap-4">
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black text-slate-500 uppercase ml-2">Dept</label>
                                    <select
                                        className="w-full bg-slate-800 border-2 border-slate-700 rounded-xl p-3 text-white font-bold text-xs"
                                        value={attendanceSession.department}
                                        onChange={(e) => setAttendanceSession({ ...attendanceSession, department: e.target.value })}
                                    >
                                        <option>B.Tech CS</option><option>B.Tech IT</option>
                                        <option>B.Tech ECE</option>
                                    </select>
                                </div>
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black text-slate-500 uppercase ml-2">Year</label>
                                    <select
                                        className="w-full bg-slate-800 border-2 border-slate-700 rounded-xl p-3 text-white font-bold text-xs"
                                        value={attendanceSession.year}
                                        onChange={(e) => setAttendanceSession({ ...attendanceSession, year: parseInt(e.target.value) })}
                                    >
                                        {[1, 2, 3, 4].map(y => <option key={y} value={y}>{y} yr</option>)}
                                    </select>
                                </div>
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black text-slate-500 uppercase ml-2">Sec</label>
                                    <select
                                        className="w-full bg-slate-800 border-2 border-slate-700 rounded-xl p-3 text-white font-bold text-xs"
                                        value={attendanceSession.section}
                                        onChange={(e) => setAttendanceSession({ ...attendanceSession, section: e.target.value })}
                                    >
                                        <option>A</option><option>B</option><option>C</option><option>All</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="w-full space-y-8 animate-in zoom-in duration-300">
                                ...
                                <div className="w-full bg-slate-950/50 rounded-2xl border border-slate-800 overflow-hidden">
                                    <div className="bg-slate-800 p-3 flex justify-between items-center">
                                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Live Scans ({scannedStudents.length})</span>
                                        <div className="h-2 w-2 bg-emerald-500 rounded-full animate-pulse"></div>
                                    </div>
                                    <div className="max-h-64 overflow-y-auto divide-y divide-slate-800">
                                        {scannedStudents.length === 0 ? (
                                            <div className="p-8 text-slate-600 text-[10px] font-black uppercase italic">Waiting for scans...</div>
                                        ) : (
                                            scannedStudents.map(s => (
                                                <div key={s.id} className="p-4 flex justify-between items-center group animate-in slide-in-from-right-4 duration-300">
                                                    <div className="flex flex-col items-start px-2">
                                                        <span className="text-white font-bold text-sm">{s.name}</span>
                                                        <span className="text-slate-500 text-[10px] font-bold uppercase">{s.roll_number}</span>
                                                    </div>
                                                    <div className="flex items-center gap-3">

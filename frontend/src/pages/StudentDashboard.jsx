...
const StudentDashboard = () => {
    ...
    // Generate QR code when payment QR data is available
    useEffect(() => {
        if (showPaymentQR && paymentQRData) {
            const canvas = document.getElementById('payment-qr-canvas');
            if (canvas) {
                QRCode.toCanvas(canvas, paymentQRData, {
                    width: 300,
                    margin: 2,
                    color: {
                        dark: '#000000',
    ...
    const downloadQR = () => {
        ...
        if (canvas) {
            ...
            link.download = `fee-payment-qr-${Date.now()}.png`;
            link.href = url;
            link.click();
        }
    };
            ...
            <div className="space-y-8 animate-in fade-in duration-500 p-8">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-4xl font-black tracking-tight text-white uppercase italic drop-shadow-lg">
                                Student Portal
                            </h1>
                            <p className="text-slate-400 font-medium mt-1">Welcome back, {profile?.name}</p>
                        </div>
                    </div>
                            ...
                            {!showPaymentQR ? (
                                <form onSubmit={handleFeePayment} className="space-y-6">
                                    ...
                                    <div className="p-6 bg-indigo-500/5 rounded-2xl border border-indigo-500/10">
                                        <p className="text-xs text-slate-400 font-medium text-center">Scan QR code to pay via UPI/Payment Gateway</p>
                                    </div>
                                    <button type="submit" className="w-full py-5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-black rounded-2xl hover:from-indigo-500 hover:to-purple-500 transition-all uppercase tracking-widest shadow-xl flex items-center justify-center gap-4">
                                        <QrCode size={20} />
                                        Generate Payment QR
                                    </button>
                                </form>
                                ...
                                <div className="space-y-6">
                                    <div className="bg-white p-8 rounded-3xl">
                                        <canvas id="payment-qr-canvas" className="mx-auto"></canvas>
                                    </div>

                                    <div className="bg-slate-950 p-6 rounded-2xl border border-slate-800">
                                        <div className="grid grid-cols-2 gap-4 text-sm">
                                            <div>
                                                <p className="text-slate-500 text-xs uppercase font-bold">Amount</p>
                                                <p className="text-white font-black text-xl">₹{payAmount}</p>
                                            </div>
                                            <div>
                                                <p className="text-slate-500 text-xs uppercase font-bold">Student</p>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <button
                                            onClick={downloadQR}
                                            className="py-4 bg-slate-800 text-white font-black rounded-2xl hover:bg-slate-700 transition-all uppercase tracking-widest flex items-center justify-center gap-2"
                                        >
                                        ...
                                        <button
                                            ...
                                            className="py-4 bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-black rounded-2xl hover:from-emerald-500 hover:to-teal-500 transition-all uppercase tracking-widest flex items-center justify-center gap-2"
                                        >
                                            <CheckCircle size={20} />
                                            Confirm
                                        </button>
                                    ...
                                    <button
                                        ...
                                        className="w-full py-3 text-slate-400 hover:text-white font-bold transition-all"
                                    >
                                        Cancel
                                    </button>
                                </div>

...
async def mark_attendance(
    ...
    # 1. Validate Active Session & Section
    from websocket_manager import manager
    session = manager.get_session(teacher_id)
    if not session:
        raise HTTPException(status_code=400, detail="No active attendance session found for this teacher")
    ...
    is_authorized = (
        student_profile.department == session['department'] and
        student_profile.year == session['year'] and
        (session['section'] == "All" or student_profile.section == session['section'])
    )
    
    if not is_authorized:
        raise HTTPException(
            status_code=403, 
            detail=f"You are not authorized for this session. Required: {session['department']} Year {session['year']} Sec {session['section']}"
        )
    ...
    # 4. Notify Teacher in Real-time
    await manager.notify_user(teacher_id, {

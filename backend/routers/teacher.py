...
async def generate_attendance_qr(
    session: schemas.AttendanceSessionStart,
    current_user: models.User = Depends(dependencies.RoleChecker(["TEACHER"]))
):
    from websocket_manager import manager
    # Register session in websocket manager
    manager.start_session(current_user.id, session.dict())
...
@router.delete("/attendance/{student_id}/{subject_id}/{hour_slot}")
